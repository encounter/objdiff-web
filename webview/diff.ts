...
export function displayDiff(
  ...
  if (diff.branch_from) {
    cb({
      type: 'basic_color',
      text: ' ~> ',
      index: diff.branch_from.branch_index,
    });
  } else {
  ...
  for (let i = 0; i < ins.arguments.length; i++) {
    ...
    switch (arg.oneofKind) {
      ...
      case 'relocation': {
        ...
        if (!reloc || !reloc.target) {
          cb({ type: 'basic', text: '<invalid>', diff_index });
          break;
        }
      ...
      case 'branch_dest':
        if (arg.branch_dest < baseAddr) {
          ...
          cb({
            type: 'branch_dest',
            address: arg.branch_dest - baseAddr,
            diff_index,
          });
        }
        break;
  ...
  if (diff.branch_to) {
    cb({
      type: 'basic_color',
      text: ' ~> ',
      index: diff.branch_to.branch_index,
    });
  }
}
